...
const Menu = () => {
  const { IsMenuOpen, setIsMenuOpen } = useActiveContext();
  return (
    <motion.aside
      className={`fixed top-0 transition-transform duration-300  px-2 overflow-y-scroll right-0 sm:w-[25rem]  block lg:hidden w-full bg-stone-950/90 backdrop-blur border-l border-gray-100/20  z-30 h-full ${
        IsMenuOpen ? "translate-x-0" : "translate-x-full"
      }`}
    >
      <button onClick={() => setIsMenuOpen(false)} className="mt-2 p-5">
        <svg
        ...
        {navlinks.map((link, i) => {
          return (
            <motion.li
              ...
              className=""
            >
              <Link href={link.hash} className="py-8 px-7 w-full capitalize">
                <span className="mr-3"> 0{i + 1}</span> {link.path}
              </Link>
            </motion.li>
          );
        })}
      ...
      <Link
        href="/resume"
        className="group flex my-4 ml-3  items-center gap-2"
        onClick={() => setIsMenuOpen(false)}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="bg-white/10 text-center mt-2 mb-2 mx-auto flex justify-center py-[3px]  rounded-lg">
        <Link
          href={"https://api.whatsapp.com/message/UT7SN2373O3NL1"}
          target="_blank"
          className="hover:bg-white/10 rounded p-2 transition duration-200"
        >
          <motion.svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-7 h-7"
